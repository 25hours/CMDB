...
class Asset(object):
    def __init__(self,request):
        ...
        self.mandatory_fields = ['sn','asset_id','asset_type'] #must contains 'sn' , 'asset_id' and 'asset_type'，检测客户端的数据是否合法
        self.field_sets = {
            'asset':['manufactory'],
    ...
    def response_msg(self,msg_type,key,msg):
        if msg_type in self.response:
            ...
            raise ValueError
    def mandatory_check(self,data,only_check_sn=False):     #强制检测数据的合法性
    ...
    def data_is_valid(self):
        ...
        if data:
            try:
                ...
                self.clean_data = data
                if not self.response['error']:      #error列表为空，表示没有错误，出错情况如jsonload失败
                    return True
            except ValueError as e:
                self.response_msg('error','AssetDataInvalid', str(e))
        else:
    ...
    def __is_new_asset(self):
        if not hasattr(self.asset_obj, self.clean_data['asset_type']):      #new asset新资产判断,self.asset_obj没有server字段代表新资产
            return True
        else:
            return False
    ...
    def _update_server(self):
        nic = self.__update_asset_component(data_source=self.clean_data['nic'],     #客户端取得的数据源用户更新对比
                                            fk='nic_set',
    ...
    def __create_server_info(self,ignore_errs=False):       #创建server表
        try:
            self.__verify_field(self.clean_data,'model',str)    #判断model字段是否存在
    ...
    def __create_or_update_manufactory(self,ignore_errs=False):     #更新或创建厂商
        try:
            self.__verify_field(self.clean_data,'manufactory',str)      #判断厂商字段是否存在
    ...
    def __create_disk_component(self):
        disk_info = self.clean_data.get('physical_disk_driver')     #列表
        if disk_info:
    ...
    def __create_nic_component(self):   #网卡
    ...
    def __create_ram_component(self):   #内存
    ...
    def __update_asset_component(self,data_source,fk,update_fields,identify_field=None):
        ...
        try:
            ...
            if hasattr(component_obj,'select_related'): # this component is reverse m2m relation with Asset model
                ...
                for obj in objects_from_db:     #循环数据库的数据
                    ...
                    if type(data_source) is list:
                        for source_data_item  in data_source:
                            key_field_data_from_source_data = source_data_item.get(identify_field)      #取得mac地址
    ...
    def __filter_add_or_deleted_components(self,model_obj_name,data_from_db,data_source,identify_field):
        ...
        data_source_key_list = set(data_source_key_list)    #利用集合将客户端数据与服务器端数据比较去重，更新
    ...
    def __delete_components(self,all_components, delete_list , identify_field ):
        ...
        for obj in all_components:      #数据对象
            val  = getattr(obj,identify_field)
            if val in delete_list:      #数据的具体的值
        ...
        for field in fields_from_db:
            ...
            if val_from_data_source:
                ...
                if val_from_db == val_from_data_source:# this field haven't changed since last update
                    ...
                    db_field = model_obj._meta.get_field(field)     #获取字段的对象
                    db_field.save_form_data(model_obj, val_from_data_source)    #更改字段
